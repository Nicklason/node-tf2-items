 ...
 * @param {boolean} [ignoreError=true] Ignore errors when updating the schema if you use a cached schema and the TF2 API is down. Defaults to `true`.
 * @param {funciton} [callback] Function to call when done
 * @throws Will throw an error if `apiKey` is not set
 */
Items.prototype.init = function (ignoreError, callback) {
    if (typeof ignoreError === 'function') {
        callback = ignoreError;
        ignoreError = true;
    } else if (ignoreError === undefined) {
        ignoreError = true;
    }
    ...
    const hasSchema = this.schema !== undefined;
