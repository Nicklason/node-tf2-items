 ...
 * @param {boolean} [item.festive] If the item is Festivized or not
 * @param {number} [item.paintkit] Skin id
 * @param {number} [item.wear]
 * @param {number} [item.quality2]
...
Schema.prototype.getName = function (item, proper=true) {
    ...
    if (item.quality2 !== null && item.quality2 !== undefined) {
        name += this.getQuality(item.quality2) + ' ';
    }
    ...
    if (item.paintkit !== null && item.paintkit !== undefined) {
        const skin = this.getSkin(item.paintkit);
        name += skin + ' ';
    } else if (schemaItem.item_quality == 15 && schemaItem.hasOwnProperty('item_slot') && ['primary', 'secondary', 'melee'].indexOf(schemaItem.item_slot) !== -1) {
    ...
    if (item.wear !== null && item.wear !== undefined) {
